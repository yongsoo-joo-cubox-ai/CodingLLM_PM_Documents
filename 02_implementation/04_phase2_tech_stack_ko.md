...
> **구현 문서 4/5** | 이전: [비용 분석](./03_cost_analysis_ko.md) | 다음: [API 레퍼런스](./05_api_reference_ko.md) | [폴더 인덱스](./README.md)

---

> **TL;DR**
> - Phase 2 핵심 기술: vLLM(추론 엔진), Rust+Loco.rs(백엔드), rmcp(MCP 서버), Coco Studio(프론트엔드)
> - 개발 항목별 기술 스택, 학습 자료, 구현 가이드를 우선순위 매트릭스와 함께 정리
> - 트랙 2(코딩 에이전트) 구현체: SecernCode(Go 1.24) — 단일 바이너리, 폐쇄망 최적화
> - MCP 서버 6종 운영 중 (xFrame5×2, Vue, Spring, React, WebSquare)
>
> **대상**: 개발자 | **소요**: ~20분 | **용어**: [용어집](../05_knowledge_base/glossary_ko.md)
...
| 백엔드 | Rust + Loco.rs | 유지 |
| **MCP 서버** | rmcp v0.8.0 (Rust) | ✅ **6개 운영 중** (xframe5×2, vue, spring, react, websquare) |
...
| **UASL 스펙** | v3 (2026-03-11) | ✅ **v1.1 → v3 업데이트** (복합 화면, 용어 정규화) |

### 트랙 2 기술 스택 (v4.4 — SecernCode, Go 기반)

| 계층 | 기술 | 용도 |
|------|------|------|
| **언어/런타임** | Go 1.24 | 단일 바이너리 컴파일, 폐쇄망 배포 |
| **CLI 프레임워크** | Cobra v1.9 + Viper v1.20 | 명령어 파싱, 설정 관리 |
| **TUI** | Bubble Tea v1.3 + Lipgloss v1.1 | 터미널 대화형 인터페이스 |
| **HTTP/API** | Go 표준 라이브러리 (net/http) | REST + SSE 스트리밍 |
| **데이터베이스** | SQLite (ncruces/go-sqlite3) + sqlc + goose | 세션/메시지/파일 관리 |
| **LLM 클라이언트** | openai-go v0.1.0-beta.2 | vLLM OpenAI-compatible API 호출 |
| **MCP** | mark3labs/mcp-go v0.17.0 | 외부 도구 연동 (stdio/SSE) |
| **코드 분석** | LSP 프로토콜 + Chroma v2 | 코드 인텔리전스, 구문 강조 |
| **빌드/배포** | GoReleaser | 크로스 플랫폼 바이너리 빌드 |

> 상세: [트랙 2 기술 전략 리서치 (부록: Go 전환 결정)](../01_strategy/05_track2_tech_strategy_ko.md)
...
> 보안 인증·모델 암호화·멀티 모델 셀렉션 상세는 [vLLM R&D 계획](./06_vllm_rd_plan_ko.md) 참조
...
MCP Servers (Framework-Specific)
  ...
  ├─ vue-compiler        ✅ 운영 중
  ├─ spring-compiler     ✅ 운영 중
  ├─ react-compiler      ✅ 운영 중
  └─ websquare-compiler  ✅ 운영 중
```

**현황**: 6개 MCP 서버 모두 운영 중 (당초 Phase 2 예정이었으나 Phase 1에서 조기 달성)
...
#### ~~C3. 멀티 프레임워크 (Spring, Vue, React)~~ ✅ Phase 1 완료

> 당초 Phase 2/3 예정이었으나 Phase 1에서 6종 전부 조기 구현 완료.
> FrameworkAdapter 트레이트 기반 MCP 서버 아키텍처로 확장성 확보.
...
| ~~C3-1. Spring MCP 서버~~ | ~~P0~~ | - | - | - | ✅ **Phase 1 완료** |
...
| `02_implementation/05_api_reference_ko.md` | API 명세 (C2, D3) |
...
| 4.4 | 2026-03-26 | Track 2 기술스택 SecernCode(Go) 기반 갱신 | 시선AI |
| 2.0 | 2026-02-12 | C1(MCP 분리) Phase 1 완료 반영, 기술 스택 현황 업데이트, 4월 일정 Spring MCP로 조정, 우선순위 매트릭스 업데이트, 리스크 재평가 | 분석팀 |
| 3.0 | 2026-03-19 | MCP 6종 반영, UASL v3 업데이트 | 분석팀 |
| 4.0 | 2026-03-19 | 트랙 2 에이전트 참고 기술 스택 추가, MoE 모델 동향 반영 | PM (주용수) |
| 4.1 | 2026-03-19 | MCP 서버 아키텍처 다이어그램 현행화 (6개 전수 운영 반영), Spring MCP 우선순위 매트릭스 완료 처리 | PM (주용수) |
| 4.2 | 2026-03-19 | vLLM R&D 교차 참조 추가 | PM (주용수) |
| 4.3 | 2026-03-19 | TL;DR 블록 추가, 파일명 넘버링 적용 | PM (주용수) |
