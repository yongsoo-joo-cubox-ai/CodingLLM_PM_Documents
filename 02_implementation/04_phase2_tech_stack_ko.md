...
> - 트랙 2(코딩 에이전트) 참고 스택: OpenCode(Go), Vercel AI SDK, MoE 모델 동향 포함
...
### 트랙 2 참고 기술 스택 (v4.0 — 에이전트 확장 시)

| 구성요소 | 버전/사양 | 비고 |
|---------|----------|------|
| **OpenCode 코어** | TypeScript + Bun 런타임 | 📋 트랙 2 착수 시 포크 |
| **HTTP API** | Hono 프레임워크 | 클라이언트/서버 분리 아키텍처 |
| **AI SDK** | Vercel AI SDK (@ai-sdk/openai-compatible) | 커스텀 Provider로 자체 sLLM 연동 |
| **에이전트 팀** | Event-driven P2P 메시징 | 다중 모델 에이전트 협업 |
| **플러그인** | JS/TS 이벤트 훅 기반 | 보안 스캐닝, 정적 분석 강제 |
| **MoE 모델** | Qwen3-235B, DeepSeek-V3.2 등 | 활성 파라미터 10% 내외, 효율적 추론 |
