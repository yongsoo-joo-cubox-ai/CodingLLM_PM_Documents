...
|------|------|------|--------------------|---------|------|
| **Infra-S0** | 2026-04~06 | 모델 가중치 암호화 + 라이선스 키 관리 + MLX 로컬 서빙 | python/secernai_crypto + cmd/keygen + cmd/license-agent + mlx_loader | PRD §4.1 (P0) | ✅ **Phase 1~6 + Phase A 완료** — Phase 5 Track 3 LicenseSDK 통합 12/12 시나리오 실측 (2026-04-10, R-BUG-1/2/3 수정). Phase 6 License Agent + KeyProvider + 3계층 라이선스 53 tests PASS (2026-04-06). Phase A MLX 로컬 3자 E2E 완료 (2026-04-11, LIC-06 embedded license_key + ADR-8 하드웨어 불일치 자동 재발급 금지 포함). Phase B(번들 패키징, 문기봉 상무 전달) 예정 |
...
| Phase 5 | W6 | E2E 테스트 + Docker 이미지 + 키 관리 | 배포 가이드, 테스트 결과 | deploy/docker/Dockerfile.vllm, tests/ | S3 연동은 Infra-S1로 연기 (MinIO AGPL v3 이슈) |
| Phase 5 Track 3 | W6 추가 | License Agent LicenseSDK 통합 (cgo + 래퍼 authenticate() 이식 + D1~D6) | 12/12 시나리오 실측, R-BUG-1/2/3 수정, Architect 2-pass APPROVED | cmd/license-agent/sdk_cgo.go, license_sdk.go, license_manager.go, reverify.go, license_state.go | ✅ 실측 완료 (2026-04-10). 상세: secern-vllm-ext/docs/phase5_track3_plan.md |
| Phase 6 | W7 | License Agent + KeyProvider + 3계층 라이선스 보호 | License Agent 바이너리, key_provider.py, 53 tests PASS | cmd/license-agent/, python/secernai_crypto/key_provider.py | ✅ 완료 (2026-04-06). 상세: secern-vllm-ext/docs/phase6_plan.md, phase6_review.md |
| Phase A | 2026-04-10~11 | MLX 로컬 3자 E2E (license-agent + mlx-serve + SecernCode + 암호화 Secern 모델) | mlx_loader 경로 B 서빙 통합, RAM disk 경로 quarantine, LIC-06 embedded license_key (-ldflags), F1(SecernCode CanReason:false + tools strip) / F2-followup(sdk_cgo.go JSON parse fix) / F3(bf16 cold boot 72.6s) fix, ADR-8(SDK 1006-1008 자동 재발급 금지), license-agent system_info 자동 생성 | python/secernai_crypto/mlx_loader.py, cmd/license-agent/embedded_key.go, scripts/build_license_agent_embedded.sh | ✅ 완료. 상세: secern-vllm-ext/.omc/plans/mlx-e2e-local-plan.md, phase-a-progress.md, phase-a-lessons-learned.md |
| Phase B | 2026-04-11 | 번들 패키징 (macOS arm64, 문기봉 상무 전달 / 내부 테스트) | tar.gz + install.sh(uv 기본 + portable Python 3.11.15 fallback), 4bit/8bit/bf16 분할/통합, Google Drive 배포, 폐쇄망 운영 OK(performOfflineAuth 폴백), LIC-06 번들 전용 키 `4G6E-W8YL-XZG7-EJNB` -ldflags 주입, start_mlx_serve.py LicenseAgentKeyProvider 암호화 경로 | scripts/build_bundle.sh + deploy/bundle-template/ | ✅ 빌드+개발자 smoke test PASS (2026-04-11, 3/3 온라인+폐쇄망). M2 맥북 최종 실측(AC-B16) 예정 |

> **투트랙 전략 (ADR-ENC-001, PRD v0.7)**: AWQ 양자화 모델에서 Tensorizer 역직렬화+추론 불안정(vLLM v0.14.0). Track A(비양자화+Tensorizer 암호화) + Track B(AWQ+safetensors 파일 암호화)로 분기. 상세: [PRD §ADR-ENC-001](./08_vllm_infra_prd_ko.md)
...
| 1.1 | 2026-04-02 | Infra-S0 상태 갱신 — Phase 2~3 기능 검증 완료, 투트랙 전략(ADR-ENC-001) 주석 추가 | PM (주용수) |
| 1.2 | 2026-04-06 | Infra-S0 Phase 6 추가 — License Agent + KeyProvider + 3계층 라이선스 보호. 기간 W7~W12, 상태 갱신 | PM (주용수) |
| 1.3 | 2026-04-10 | Infra-S0 Phase 6 상태 갱신 — MLX 트랙에서 LicenseSDK 통합 결정. 기존 Ed25519 자체 구현 → LicenseSDK C API 전환 (해석 C). 상세: secern-vllm-ext/docs/mlx_phase5_licensesdk_integration.md | PM (주용수) |
