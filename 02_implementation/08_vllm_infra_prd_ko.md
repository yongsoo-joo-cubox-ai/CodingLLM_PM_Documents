...
1. **모델 가중치가 디스크에 평문 저장**: 파인튜닝된 sLLM(GPT-OSS 20B, Qwen3-Coder 등)은 핵심 IP이나, 물리적 접근 시 유출 가능
2. **단일 vLLM 인스턴스 = 단일 모델**: 이종 모델(7B QA용 + 32B 코드생성용)의 동시 운영 및 동적 라우팅 불가
...
[개발자/에이전트]
       |
  [인증 게이트웨이 (FastAPI)] -- JWT 검증, RBAC, 감사 로그 (단일 인증 지점)
       |
...
(32B코드)  (7B QA)     (선택적: --middleware로 심층 방어)
  |         |
...
| **접근 제어** | LiteLLM 앞단 FastAPI 인증 게이트웨이에서 JWT/SSO 검증, RBAC 인가, 전 요청 감사 로깅. Rate Limiting은 LiteLLM 가상 키 시스템에 위임. vLLM 직접 접근은 네트워크 정책으로 차단 (선택적 심층 방어로 vLLM --middleware 병용 가능) |
...
- FastAPI ASGI 인증 게이트웨이: JWT 토큰 검증, RBAC, 감사 로깅 (Rate Limiting은 LiteLLM 가상 키에 위임)
...
| ID | 요구사항 | 상세 | 수용 기준 |
|----|---------|------|----------|
| ENC-01 | Tensorizer 직렬화 | 파인튜닝된 HuggingFace 포맷 모델을 `tensorize_vllm_model.py`로 Tensorizer 포맷(.tensors)으로 변환 | GPT-OSS 20B, Qwen2.5-Coder-32B-AWQ 2종 이상 변환 성공 |
...
| DEC-03 | 로딩 성능 오버헤드 | 암호화된 모델의 로딩 시간이 평문 safetensors 대비 **2배 이내** | Phase 1 W1에서 평문 GPT-OSS 20B 로딩 시간을 기준선으로 측정 후, 해당 값의 2배를 상한으로 설정. 벤치마크 측정: GPT-OSS 20B 기준 로딩 시간 비교 |
...
| LIT-02 | config.yaml 모델 매핑 | 최소 2종 모델(코드생성 32B + QA/요약 7B)을 가상 모델명(model_name)으로 매핑 | `secern-coder-32b`, `secern-light-8b` 호출 시 올바른 백엔드로 라우팅 (예시, 실제 모델명은 .secerncode.json과 동기화) |
...
| LIT-06 | 자동 폴백 | 주력 모델 서버 장애 시 대체 모델로 자동 전환 | 32B 서버 다운 시 7B로 폴백, 응답 반환 확인 (degraded mode) |
...
| ID | 요구사항 | 상세 | 수용 기준 |
|----|---------|------|----------|
| AUTH-01 | LiteLLM 앞단 FastAPI 인증 게이트웨이 | 독립적인 Python 패키지(`secernai-auth`)로 개발. LiteLLM 프록시 앞에 FastAPI ASGI 게이트웨이로 배치하여 단일 인증 지점(Single Authentication Point)을 제공. vLLM의 `--middleware`는 선택적 심층 방어(Defense-in-Depth)로만 활용 (LiteLLM 우회 방지, 필수 아님) | LiteLLM 프록시를 경유하는 모든 요청이 게이트웨이에서 인증됨. vLLM 직접 접근은 네트워크 정책으로 차단 |
...
| 항목 | 기준 |
|------|------|
| 암호화 모델 로딩 시간 | 평문 모델 대비 **2배 이내** (GPT-OSS 20B 기준) |
...
| 항목 | 기준 |
|------|------|
| vLLM 버전 | >= 0.6.0 (tool calling 지원 필수), 현재 최신 안정 버전 호환 |
| Python | >= 3.10 |
...
| 항목 | 기준 |
|------|------|
| 에어갭 설치 | 모든 의존성 pip wheel + Docker 이미지를 오프라인 패키지로 제공 |
...
| 주차 | 작업 | 담당 | 산출물 |
|------|------|------|--------|
| W1 | **VRAM 실측 벤치마크** + Tensorizer 환경 구축 | 주용수 | VRAM 실측 보고서 (32B+7B 동시 서빙 가능 여부), 평문 GPT-OSS 20B 로딩 시간 기준선 측정 |
...
**마일스톤 체크포인트**:
- **W1 게이트**: VRAM 실측 완료. 32B+7B 동시 서빙 가능 여부 확인 → OQ-4 결정
- **W5~6**: GPT-OSS 20B 암호화 모델이 vLLM에서 정상 추론 동작, 로딩 시간 기준선의 2배 이내
...
| 주차 | 작업 | 담당 | 산출물 |
|------|------|------|--------|
| W13 | **Redis 인프라 확보** (기존 운영 중이면 접속 확인, 없으면 Docker로 구축) + ASGI 미들웨어 패키지 개발 착수 | 주용수 | Redis 가용성 확인 보고, secernai-auth 패키지 스캐폴딩 |
| W13-14 | 인증 게이트웨이 패키지 개발 (JWT 검증 + RBAC) | 주용수 | secernai-auth 패키지 |
| W15-16 | Redis 연동 (블랙리스트) + 감사 로깅 | 주용수 | 감사 로그 스키마, 운영 가이드 |
| W17 | 인증 게이트웨이 → LiteLLM → vLLM E2E 통합 테스트 + 부하 테스트. 선택적 vLLM --middleware 심층 방어 검증 | 주용수 | 통합 테스트 결과, 성능 벤치마크 |
| W18 | SSO/SAML 연동 인터페이스 설계 + 라이선스 서버 연동 프로토타입 | 주용수 | 인터페이스 명세서, 프로토타입 |

**마일스톤 체크포인트**: 인증 게이트웨이에서 JWT 인증 + RBAC가 동작, LiteLLM 경유 전 요청에 감사 로그 정상 기록
...
| 암호화 로딩 오버헤드 | 평문 대비 **2배 이내** | GPT-OSS 20B 로딩 시간 벤치마크 |
...
| R3 | GPU 메모리 부족으로 복수 vLLM 인스턴스 운영 불가 | 중간 | 높음 | AWQ/INT4 양자화로 메모리 절감. 모델 가중치만 ~26GB(32B AWQ ~18GB + 7B ~8GB)이나, **KV Cache + 활성화 버퍼 포함 시 60~80GB 이상 예상 (실측 필요)**. A100 80GB에서도 동시 서빙이 빡빡할 수 있음. Phase 1 W1에 VRAM 실측 벤치마크 실시 후, Phase 2 착수 시 Go/No-Go 게이트에서 멀티 인스턴스 vs Multi-LoRA vs 단일 모델 A/B 전환 결정 |
...
| OQ-4 | GPU 서버에서 32B + 7B 두 모델 동시 운영이 현재 GPU 메모리 내에서 가능한가? 가중치만 ~26GB이나 KV Cache 포함 시 60~80GB 이상 예상. **Phase 1 W1 VRAM 실측 벤치마크로 확인** | LiteLLM | **Phase 1 W1 (4월 초)** | 멀티 인스턴스 vs Multi-LoRA vs 단일 모델 A/B 전환 결정. 하드웨어 추가 구매 또는 모델 조합 변경 |
...
### Follow-ups
- Phase 1 완료 후: 금융권 보안 심사 대응 가능 여부 평가
- Phase 2 완료 후: Track 1 LiteLLM 마이그레이션 여부 결정
- Phase 3 완료 후: 실제 고객사 환경에서 E2E 보안 테스트 실시
