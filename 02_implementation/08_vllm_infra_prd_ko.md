...
(32B코드)  (30B-MoE QA) (선택적: --middleware로 심층 방어)
  |         |
[암호화된 가중치] -- Track A: Tensorizer + libsodium (비양자화)
                     Track B: safetensors + 파일 레벨 libsodium (AWQ) → ADR-ENC-001
```

| 항목 | 목표 상태 |
|------|----------|
| **모델 보안** | **Track A** (비양자화): Tensorizer 텐서 레벨 암호화, GPU 적재 시 On-the-fly 복호화. **Track B** (AWQ 양자화): safetensors + libsodium 파일 레벨 암호화, tmpfs 복호화 후 로딩 (ADR-ENC-001). 공통: 디스크에 암호문만 존재 |
...
- 파인튜닝된 sLLM 가중치를 libsodium으로 암호화하여 보호: **Track A**(비양자화) Tensorizer On-the-fly 복호화, **Track B**(AWQ 양자화) safetensors + 파일 레벨 암호화 → tmpfs 복호화 후 로딩 (ADR-ENC-001)
...
> **Track 참조**: 아래 ENC/DEC 요구사항은 **Track A**(비양자화+Tensorizer) 기준. AWQ 양자화 모델의 **Track B** 요구사항은 ADR-ENC-001에서 정의. Track B에서는 ENC-01이 safetensors 파일 암호화, DEC-01이 tmpfs 복호화 후 safetensors 로딩, DEC-02가 tmpfs 복호화+shred로 대체됨.
...
| ENC-03 | S3 호환 내부 스토리지 연동 | ~~MinIO~~ → **Infra-S1로 연기** (AGPL v3 라이선스 이슈). 현재 로컬 파일 경로 운영. Infra-S1에서 SeaweedFS(Apache 2.0) 또는 NFS 검토 | Infra-S1에서 재정의 |
...
| DEC-03 | 로딩 성능 오버헤드 | 암호화된 모델의 로딩 시간이 평문 safetensors 대비 **2배 이내** | Phase 1 W1에서 평문 Qwen2.5-32B-AWQ 로딩 시간을 기준선으로 측정 완료 (**35.2초, 68.3 tok/s**). 해당 값의 2배(70.4초)를 상한으로 설정. **실측(v0.8)**: Track A 12.5초 ✅ PASS / Track B 128.9초 ❌ FAIL (복호화 71.5초 + 로딩 57.4초, Phase 4 최적화 필요). **실측(v0.9)**: Track B Phase 4 복호화 19.1초(ProcessPool) + 로딩 60초 = 79.1초, 보수적 상한(95초) ✅ PASS |
| DEC-04 | 추론 성능 무영향 | GPU VRAM 적재 완료 후 추론 성능(tokens/sec)은 평문 모델과 동일 | 동일 프롬프트 100회 벤치마크에서 throughput 차이 1% 미만. **실측(v1.0)**: 암호화 68.5 tok/s vs 기준선 68.3 tok/s = +0.3% ✅ PASS |
...
#### 4.1.4 라이선스 보호 (Phase 6 — ✅ 2026-04-06 완료, Phase A 2026-04-11 보강)

> IDP-02(§4.3.4 라이선스 서버 연동)는 **Infra-S2**의 Gateway 인증 범위로, API 수준의 동시 접속/토큰 사용량 제한을 다룬다.
> LIC는 **Infra-S0**의 모델 IP 보호 + 키 배포 범위로, 모델 복호화 자체를 라이선스에 연동한다.

| ID | 요구사항 | 상세 | 수용 기준 |
|----|---------|------|----------|
| LIC-01 | KeyProvider 추상화 | 키 획득 경로를 인터페이스(ABC)로 분리. 파일/License Agent/향후 KMS 교체 가능 구조 | 기존 FileKeyProvider로 Phase 5 E2E 테스트 전체 PASS (하위 호환) |
| LIC-02 | License Agent 키 브로커 | Go 바이너리가 Ed25519 라이선스 서명 검증 + SecretBox Key Wrapping 해제 후 localhost API로 키 제공 | 유효 라이선스: 키 제공(200). 만료 라이선스: 거부(403) |
| LIC-03 | 3계층 라이선스 검증 | Gateway(진입 차단)/LiteLLM(사용량 제어)/vLLM(키 획득 차단) 각 계층에서 독립적 라이선스 검증 | 어느 한 계층을 우회해도 나머지 계층이 차단 |
| LIC-04 | Key Wrapping | model_key를 license_key로 SecretBox 암호화하여 배포(72바이트: nonce 24 + ciphertext 32 + MAC 16). 고객사 디스크에 평문 키 미잔존 | wrapped_key.bin 단독으로 model_key 복원 불가 |
| LIC-05 | 듀얼 모드 (에어갭/온라인) | 에어갭: license.dat 로컬 Ed25519 서명 검증 + 시스템 시간. 온라인: 로컬 검증 + 시선AI 서버 heartbeat | 에어갭 모드: 외부 통신 0건. 온라인 모드: heartbeat 정상 동작 |
| LIC-06 | K-6 Embedded License Key (옵션 B) | license_key를 Go 바이너리 빌드 시점에 `-ldflags "-X main.embeddedLicenseKeyB64=<base64>"` 주입. 디스크에 license_key 파일 미잔존 (개발용 파일 폴백 유지). 키 교체 = 빌드 인자 교체 + 재빌드 (Phase A 2026-04-11 구현) | `embedded_key.go GetLicenseKey()` 실행 시 runtime 파일 우선 → embedded 폴백. 단위 테스트 PASS. 문기봉 상무 배포 번들은 별도 키로 재빌드 |

**ADR-8 (2026-04-11)**: SDK 에러 코드 1006-1008 (HW/MAC/DEVICE MISMATCH) 자동 재발급 금지. 근거: LMS create API 가 unconditional 이므로 MAC 불일치 시 자동 재발급을 허용하면 license_key 유출만으로 임의 기기에 라이선스가 자동 발급되는 보안 허점 발생. 운영 절차: license.dat 삭제 후 재시작 → "Failed to open" 경로로 재발급. 상세: `secern-vllm-ext/.omc/plans/phase-a-lessons-learned.md §7`, `cmd/license-agent/license_manager.go isHardwareMismatch`.
...
| 0.7 | 2026-04-02 | **투트랙 암호화 전략** — ADR-ENC-001 추가, §2.2/§3.1/§4.1 투트랙 반영, OQ-11 PoC 3건 실패 결과, 팩트 체크("불가능"→"불안정") 수정, Track A(비양자화+Tensorizer) + Track B(AWQ+safetensors) 구조 확정 | PM (주용수) |
| 0.8 | 2026-04-02 | DEC-03 실측값 반영 — Track A 12.5초 PASS, Track B 128.9초 FAIL (Phase 4 최적화 필요) | PM (주용수) |
