...
| DEC-03 | 로딩 성능 오버헤드 | 암호화된 모델의 로딩 시간이 평문 safetensors 대비 **2배 이내** | Phase 1 W1에서 평문 Qwen2.5-32B-AWQ 로딩 시간을 기준선으로 측정 완료 (**35.2초, 68.3 tok/s**). 해당 값의 2배(70.4초)를 상한으로 설정. **실측(v0.8)**: Track A 12.5초 ✅ PASS / Track B 128.9초 ❌ FAIL (복호화 71.5초 + 로딩 57.4초, Phase 4 최적화 필요) |
