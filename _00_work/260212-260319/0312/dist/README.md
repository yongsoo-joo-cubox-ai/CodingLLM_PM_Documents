...
- Renamed composite purposes for consistency: `list_detail_panel` → `split_view`, `tabbed_detail` → `tabbed_view`
- Updated composition docs, child properties table, and purpose-composition constraints to use new names
- `tab_label` description updated to reference `tabbed_view`

**CDS Schema (cds_v1.schema.json)**
- Renamed `detected_screen_type` values: `editor` → `form`, `detail` → `view`, `list_detail_panel` → `split_view`, `tabbed_detail` → `tabbed_view`
- Old names retained in enum for backward compatibility (engine normalizes at parse time)
...
- Added composite purposes: `master_detail`, `split_view`, `tabbed_view`
