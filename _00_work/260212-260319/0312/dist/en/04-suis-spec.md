...
| `split_view` | List on left + detail panel on right (composite) |
| `tabbed_view` | Parent form above + tabs with child grids below (composite) |
...
The three composite purposes -- `master_detail`, `split_view`, and `tabbed_view` -- combine a parent entity with one or more child entities on a single screen. Use the optional `composition` property to declare child entities and their relationship to the parent.
...
| `tab_label` | no | Tab label (for `tabbed_view` layout) |
...
| `split_view` | 0 | Single entity -- list on left, detail on right |
| `tabbed_view` | 1 or more | Parent form above, tabs with child grids below |
