...
| `split_view` | 왼쪽 목록 + 오른쪽 상세 패널 (복합) |
| `tabbed_view` | 상위 폼 + 하위 그리드 탭 (복합) |
...
세 가지 복합 목적 -- `master_detail`, `split_view`, `tabbed_view` -- 은 하나의 화면에서 상위 엔티티와 하나 이상의 하위 엔티티를 결합합니다. 선택적 `composition` 속성을 사용하여 하위 엔티티와 상위와의 관계를 선언합니다.
...
| `tab_label` | 아니오 | 탭 라벨 (`tabbed_view` 레이아웃용) |
...
| `split_view` | 0개 | 단일 엔티티 -- 왼쪽 목록, 오른쪽 상세 |
| `tabbed_view` | 1개 이상 | 상위 폼 위, 하위 그리드 탭 아래 |
