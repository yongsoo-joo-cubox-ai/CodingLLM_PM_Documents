...
| `master_detail` | 상위 폼 + 하위 그리드 (복합) |
| `list_detail_panel` | 왼쪽 목록 + 오른쪽 상세 패널 (복합) |
| `tabbed_detail` | 상위 폼 + 하위 그리드 탭 (복합) |
...
`collection` 또는 `single` 표시 블록의 각 필드는 `name`으로 엔티티 속성을 참조합니다. `summary` 블록에서는 직접적인 엔티티 속성이 아닌 집계 지표를 나타내는 파생된 이름(예: `total_count`, `pending_amount`)을 사용할 수 있습니다. 나머지는 모두 어댑터가 사용하거나 무시할 수 있는 선택적 힌트입니다.
...
`format`이 `enum`인 경우, `values` 속성으로 허용되는 값 코드를 선언할 수 있습니다:
...
값은 **코드만** 해당합니다 -- JSON Schema `enum` 및 SQL `CHECK` 제약 조건에 맞는 단순 문자열입니다. 사람이 읽을 수 있는 라벨은 SUIS의 일부가 아닙니다. 라벨은 Domain Graph의 `enum_labels` 딕셔너리에서 별도로 관리되며, 컴파일 전에 엔진이 주입합니다.

어댑터는 `values`를 사용하여 드롭다운/선택 컨트롤을 채웁니다. `enum` 필드에 `values`가 없으면, 어댑터는 텍스트 입력이 아닌 빈 드롭다운을 렌더링해야 합니다.
...
| `dropdown` | 옵션 목록에서 선택 |
| `select` | 옵션 목록에서 선택 (`dropdown`의 별칭) |
...
## 복합 구성(Composition) -- 복합 화면

세 가지 복합 목적 -- `master_detail`, `list_detail_panel`, `tabbed_detail` -- 은 하나의 화면에서 상위 엔티티와 하나 이상의 하위 엔티티를 결합합니다. 선택적 `composition` 속성을 사용하여 하위 엔티티와 상위와의 관계를 선언합니다.
...
### 하위 속성(Child Properties)

| 속성 | 필수 | 설명 |
|------|------|------|
| `entity` | 예 | 하위 엔티티 이름 (Entity Spec에서 참조) |
| `relation` | 예 | 상위와 하위를 연결하는 관계 이름 |
| `foreign_key` | 아니오 | 상위를 가리키는 하위의 외래 키 필드 |
| `id_field` | 아니오 | 하위 엔티티의 기본 키 필드 |
| `api_base_url` | 아니오 | 하위 CRUD 작업을 위한 API 기본 URL |
| `tab_label` | 아니오 | 탭 라벨 (`tabbed_detail` 레이아웃용) |
| `display` | 아니오 | 하위 엔티티 필드가 포함된 표시 블록 |

### 목적-복합 구성 제약

| 목적 | 하위 수 | 비고 |
|------|---------|------|
| `master_detail` | 정확히 1개 | 상위 폼 위, 하위 그리드 아래 |
| `list_detail_panel` | 0개 | 단일 엔티티 -- 왼쪽 목록, 오른쪽 상세 |
| `tabbed_detail` | 1개 이상 | 상위 폼 위, 하위 그리드 탭 아래 |
...
**Entity Spec** -- `subject.domain`은 실제 엔티티를 이름으로 참조해야 합니다. `collection`과 `single` 표시 블록의 모든 필드 `name`은 해당 엔티티의 속성 또는 관계와 일치해야 합니다. `summary` 블록은 예외로, 엔티티에 존재하지 않는 파생된 지표 이름(예: `total_count`)을 포함할 수 있습니다. Entity Spec이 `task`를 `title`, `status`, `due_date`, `assignee_id` 속성(및 `assignee` 관계)으로 정의했다면, SUIS의 collection 및 single 블록에서는 바로 그 이름들을 사용합니다.
