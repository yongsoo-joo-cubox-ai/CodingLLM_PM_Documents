...
| `master_detail` | Parent form above + child grid below (composite) |
| `list_detail_panel` | List on left + detail panel on right (composite) |
| `tabbed_detail` | Parent form above + tabs with child grids below (composite) |
...
Each field in a `collection` or `single` display block references an entity attribute by `name`. In `summary` blocks, fields may use derived names (e.g., `total_count`, `pending_amount`) that represent aggregated metrics rather than direct entity attributes. Everything else is optional hints that adapters can use (or ignore).
...
When `format` is `enum`, you can declare the allowed value codes using the `values` property:
...
Values are **codes only** -- plain strings aligned with JSON Schema `enum` and SQL `CHECK` constraints. Human-readable labels are NOT part of SUIS. Labels are managed separately in the Domain Graph's `enum_labels` dictionary and injected by the engine before compilation.

Adapters use `values` to populate dropdown/select controls. When `values` is absent for an `enum` field, adapters render an empty dropdown (not a text input).
...
| `select` | Pick from a list of options (alias for `dropdown`) |
...
## Composition (Composite Screens)

The three composite purposes -- `master_detail`, `list_detail_panel`, and `tabbed_detail` -- combine a parent entity with one or more child entities on a single screen. Use the optional `composition` property to declare child entities and their relationship to the parent.
...
screens:
  order_master_detail:
    subject:
      domain: order
      purpose: master_detail
      title: "Order Management"

    display:
      primary:
        type: single
        fields: [ ... ]          # Parent entity fields
    ...
    composition:
      children:
        - entity: order_item
          ...
          id_field: id
          api_base_url: /api/order_item
          tab_label: "Order Items"
          display:
            type: collection
            fields: [ ... ]      # Child entity fields
```
...
|----------|----------|-------------|
| `entity` | yes | Child entity name (from Entity Spec) |
| `relation` | yes | Relation name linking parent to child |
| `foreign_key` | no | Foreign key field on child pointing to parent |
| `id_field` | no | Primary key field of the child entity |
| `api_base_url` | no | API base URL for child CRUD operations |
| `tab_label` | no | Tab label (for `tabbed_detail` layout) |
| `display` | no | Display block with child entity fields |

### Purpose-Composition Constraints

| Purpose | Children | Notes |
|---------|----------|-------|
| `master_detail` | Exactly 1 | Parent form above, child grid below |
| `list_detail_panel` | 0 | Single entity -- list on left, detail on right |
| `tabbed_detail` | 1 or more | Parent form above, tabs with child grids below |
...
**Entity Spec** -- `subject.domain` must name a real entity. Every field `name` in `collection` and `single` display blocks must match an attribute or relation on that entity. `summary` blocks are an exception -- they may contain derived metric names (e.g., `total_count`) that do not exist on the entity. If your Entity Spec defines `task` with attributes `title`, `status`, `due_date`, and `assignee_id` (plus a relation `assignee`), those are the names you use in SUIS collection and single blocks.
