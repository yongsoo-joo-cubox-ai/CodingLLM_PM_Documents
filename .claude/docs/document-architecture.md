...
#### 공유 인프라 (secern-vllm-ext)

| 구분 | 공유 인프라 (secern-vllm-ext) |
|------|--------------------------|
| **전략·로드맵** | PM 레포 `02_implementation/09_vllm_infra_roadmap_ko.md` |
| **기술 상세** | `secern-vllm-ext/` 서브모듈 `docs/` (stage0_spec, stage1_spec, stage2_spec) |
...
| secern-vllm-ext (vLLM) | `Infra-` | Infra-S0, Infra-S1, Infra-S2 |
...
| **PM → secern-vllm-ext** | 상대경로 링크 (`../secern-vllm-ext/docs/...`) | 서브모듈이 PM 안에 포함 |
| **secern-vllm-ext → PM** | **텍스트로 문서명만 참조** (링크 없음) | 독립 레포, PM 경로 접근 불가 |
