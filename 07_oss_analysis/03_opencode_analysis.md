...
| **보안등급** | 일반 |
| **작성** | Secern AI |
| **분석 대상 버전** | v1.3.0 (커밋 `4c27e7fc6` 기준) |
...
│  │                                                            │ │
│  │  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐ │ │
│  │  │ OpenCode CLI  │    │ OpenCode TUI │    │ OpenCode App │ │ │
│  │  │ (터미널)       │    │ (Ink/SolidJS) │    │ (데스크톱)    │ │ │
│  │  └──────┬───────┘    └──────┬───────┘    └──────┬───────┘ │ │
│  │         └───────────────────┼───────────────────┘         │ │
│  │                             │                              │ │
│  │                    ┌────────▼────────┐                     │ │
...
이 문서와 `05_track2_tech_strategy_ko.md`의 역할 분담:

| 항목 | 이 문서 (03_opencode_analysis) | 05_track2_tech_strategy_ko |
|------|-------------------------------|---------------------------|
| **목적** | OpenCode 코드베이스 자체의 이해 | 트랙 2 시장/기술 전략 수립 |
| **관점** | 오픈소스 분석 (내부 구조, 모듈, 코드 흐름) | 경쟁 솔루션 비교 + 전략적 포지셔닝 |
| **깊이** | 코드 레벨 (파일 경로, 함수 호출 체인) | 기능 비교표 + 전략 방향 |
| **갱신 주기** | OpenCode 버전 업데이트 시 | 시장/전략 변화 시 |

### 7.6 교차 참조

- 트랙 2 기술 전략 리서치: [`01_strategy/05_track2_tech_strategy_ko.md`](../../01_strategy/05_track2_tech_strategy_ko.md) — OpenCode 포함 경쟁 솔루션 비교, 전략 포지셔닝
- 구현 로드맵: [`02_implementation/01_roadmap_ko.md`](../../02_implementation/01_roadmap_ko.md) — 트랙 2 코딩 에이전트 일정
- vLLM 심층 분석: [`01_vllm_analysis.md`](./01_vllm_analysis.md) — OpenCode가 사용하는 추론 엔진 백엔드
- LiteLLM 심층 분석: [`02_litellm_analysis.md`](./02_litellm_analysis.md) — OpenCode가 LLM 호출 시 경유하는 프록시 계층
- 프로젝트 용어집: [`05_knowledge_base/glossary_ko.md`](../../05_knowledge_base/glossary_ko.md) — OpenCode, MCP, 코딩 에이전트 등 용어 정의
